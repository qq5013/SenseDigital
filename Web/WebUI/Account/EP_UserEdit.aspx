<%@ Page Language="C#" AutoEventWireup="true" CodeFile="EP_UserEdit.aspx.cs" Inherits="WebUI_Account_EP_UserEdit" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#txtPersonID').bind('dblclick', function () { GetOtherJsonValue('EP_Person', '<%=cnKey%>', 'PersonID,PersonName'); });
            $('#txtPersonID').bind('change', function () { getBaseData('EP_Person', '<%=cnKey%>', this.value, 'PersonID,PersonName'); });
            if ('<%=UserName%>' == '')
                $('#trPwd').show();
            else
                $('#trPwd').hide();
        ...
        function Save() {
            ...
            if ("<%= UserName%>" == "") {
                var exist = IsExists("<%= FormID%>", $("#txtUserName").val(), '<%=cnKey%>');
                if (exist) {
        ...
        function Cancel() {
            if (confirm("<%=Resources.Resource.Question_Cancel %>"))
                location.href = "EP_User.aspx?FormID=<%= FormID%>";
            return false;
        }
        </script>
</head>
        ...
        <table class="maintable" cellspacing="0" cellpadding="0" width="100%" align="center" bordercolor="#ffffff" border="1" >			
					...
					<tr>
						...
						<td width="35%" colspan="3">&nbsp;
                            <asp:DropDownList ID="ddlEnterpriseID" runat="server" AutoPostBack="True" 
                                Width="96%">
                            </asp:DropDownList>
                            &nbsp;</td>
