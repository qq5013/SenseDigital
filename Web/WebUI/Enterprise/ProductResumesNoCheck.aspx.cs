...
public partial class WebUI_Enterprise_ProductResumesNoCheck : BasePage
{
    int PageSize = 15;
    ...
    private void BindGrid(int pageIndex)
    {
        ...
        {
            if (this.GridView1.Rows[0].Cells[17].Text.Replace("&nbsp;", "").Trim().Length > 0)
            {
    ...
    //            else
    //                dal.Delete(Session["EnterpriseID"].ToString(), "ResumeID='" + hk.Text + "'");
    //        }
    //    }
    //    SetBtnEnabled("");
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < this.GridView1.Rows.Count; i++)
        {
            ...
            {
                HyperLink hk = (HyperLink)(this.GridView1.Rows[i].FindControl("HyperLink1"));
                iReturn = sdal.GetBillCanBeEdit(FormID, Session["EnterpriseID"].ToString(), "ResumeID='" + hk.Text + "'");
    ...
    {
        string strFileName = ConfigurationManager.AppSettings["UploadPath"] + this.ddlEnterpriseID.SelectedValue + @"\Resume\Prod_resume.xls";
        if (!System.IO.File.Exists(strFileName))
        {
            JScript.Instance.ShowMessage(this.updatePanel, Resources.Resource.EP_ProdcutResumeExcelNotExists);
            return;
        }
        Js.BLL.Enterprise.CheckDal dal = new Js.BLL.Enterprise.CheckDal(FormID, cnKey);
        DataTable dt = dal.ImportProductResume1(Session["EnterpriseID"].ToString(), Session["User"].ToString());

        DataTable dtTemp = dal.GetProductResumeTemp("1=2");
        ...
        dal.SaveProductResume(dtTemp, Session["User"].ToString());
        SetBtnEnabled("");
    }
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        SetBtnEnabled("");
        JScript.Instance.ShowMessage(this.updatePanel, Resources.Resource.EP_ProductResumeUploadSuccess);
    }
